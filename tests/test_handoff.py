...
class HandoffIntegrationTest(unittest.TestCase):
    ...
    def test_initialize_audit_prompts_and_hook_are_consistent(self):
        ...
        self.assertIn("## Context Primer", work_prompt)
        self.assertIn("ChatGPT Research Controller 与 Codex 本地执行端", work_prompt)
        self.assertIn("Research question: Not yet audited.", work_prompt)
        self.assertNotIn("- Target outcome:", work_prompt)
        self.assertNotIn("- Primary direction:", work_prompt)
        self.assertIn("另用一个短段概括目标贡献、核心方法链", work_prompt)
        self.assertIn("新对话仍须从 GitHub 核验可变状态", work_prompt)
        self.assertIn("Current Chat-Codex Transaction", work_prompt)
        self.assertIn("权威文件无法恢复的用户决定", work_prompt)
        self.assertIn("不要复制 Project Instructions 或完整 Codex Goal", work_prompt)
    ...
    def test_work_contract_is_format_not_mandatory_review_state_machine(self):
        ...
        self.assertIn("compact Context Primer and the current Chat-Codex transaction", normalized)
        self.assertIn("It may summarize authority for orientation", normalized)
        ...
        self.assertIn("compact Context Primer and the current Chat-Codex transaction", skill)
        self.assertIn("Do not copy Project Instructions or the full Codex Goal", skill)
